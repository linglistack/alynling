...
function(req, res) {
  tryCatch({
    ...
    # Determine lookback_window dynamically, based on the length of the time series and periodicity of the data

    n_data = nrow(data)
    n_time = data[[time_id]] %>% unique() %>% length()
    ...
    periodicity <- safe_detect_periodicity(data[[Y_id]])
    pre_treatment_length <- n_time - treatment_length
    lookback_window = max(round(pre_treatment_length/10), periodicity, 5)
    print(periodicity)
